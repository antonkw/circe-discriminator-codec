...
package io.github.antonkw.circe.adt.codec.tests
...
class JsonTaggedAdtCodecImplTestSuite extends AnyFlatSpec {
  ...
  "A codec" should "be able to create event type" in {
    type Enc[T] = Encoder.AsObject[T]
    type Dec[T] = Decoder[T]
    type Codec[T] = ( Enc[T], Dec[T] )
    implicit val ( e: Enc[TestEvent], d: Dec[TestEvent] ) =
      JsonTaggedAdtCodec.createEventTypeCodec[TestEvent]

    val testEvent: TestEvent = TestEvent1( "test" )
    val json: String = testEvent.asJson.dropNullValues.noSpaces

    assert( json.contains( """"event_type":"ev1"""" ) )
  }
